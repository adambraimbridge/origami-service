...
function origamiService(options) {
	...
	let metrics = null;
	if (options.graphiteAppUUID) {
		if (process.env.FT_GRAPHITE_APP_UUID !== options.graphiteAppUUID) {
			process.env.FT_GRAPHITE_APP_UUID = options.graphiteAppUUID;
		}
		metrics = new Metrics();
		metrics.init({
		...
		options.log.warn('Warning: metrics are not being recorded for this application. Please provide a FT_GRAPHITE_APP_UUID environment variable');
	}
